...
for image in images:
    ...
    image_thresh, contours, hierarchy = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for xx in range(9):
        ...
        rot_mat = cv2.getRotationMatrix2D((x, y), angle - xx*10, 1.0)
        ...
        image_grayy = cv2.cvtColor(image, cv2.COLOR_BAYER_RG2RGBA)
        mask = np.zeros(image_gray.shape, np.uint8)

        resa = np.array(image_grayy)
        resb = Image.fromarray(resa)
        resb.save("miecze/{:1}".format(num) + "{:1}".format(xx)+".png")
